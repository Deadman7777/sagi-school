...
def create_production_if_needed():
    """Crée production.py depuis example si absent — Windows uniquement."""
    if sys.platform != 'win32':
        return
    
    base = os.path.dirname(os.path.abspath(__file__))
    settings_dir = os.path.join(base, 'config', 'settings')
    prod_file    = os.path.join(settings_dir, 'production.py')
